...
from speedrunning_plms.data.download import get as ensure_hf_file
...
from speedrunning_plms.training.optimizers import (
    apply_muon_momentum_warmup,
    build_optimizers,
    build_schedulers,
)
...
def build_code_snapshot(script_path: str | None = None) -> str:
    root = Path.cwd()
    snapshot_parts = []
    candidate_paths = [
        Path(script_path) if script_path is not None else Path(sys.argv[0]),
        root / "entrypoint_setup.py",
        root / "optimizer.py",
        root / "data" / "dataloading.py",
        root / "model" / "utils.py",
        root / "model" / "attention.py",
        root / "model" / "model.py",
    ]
    candidate_paths.extend(sorted((root / "src" / "speedrunning_plms").glob("**/*.py")))
    for path in candidate_paths:
        try:
            snapshot_parts.append(Path(path).read_text(encoding="utf-8"))
        except OSError:
            continue
    return "\n".join(snapshot_parts)


def set_code_snapshot(snapshot: str) -> None:
    global code
    code = snapshot
...
class Trainer:
    ...
    def init_optimizers(self):
        ...
        return build_optimizers(self.model, self.args, print_fn=self.print0)
    
    def init_schedulers(self):
        self.print0("Initializing schedulers...")
        return build_schedulers(self.optimizers, self.args)
    ...
    def train_step(self, step):
        ...
        if self.args.use_muon:
            apply_muon_momentum_warmup(
                self.optimizers[-1],
                step=step,
                warmup_steps=self.args.muon_momentum_warmup_steps,
            )
...
def main():
    ...
    if args.wandb_token and os.environ.get('WANDB_AVAILABLE') == 'true':
